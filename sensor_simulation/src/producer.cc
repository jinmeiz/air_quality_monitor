...
int main(int argc, char** argv) {
  ...
  // 1st argument: Prometheus server address (URL)
  // 1st argument: producer address (URL)
  // 2nd argument: producer port number
  // 3rd argument: sensor id
  if (argc != 5) {
    std::cout << "Inputs are not correct (they should be "
        << "Prometheus server address, producer address, producer port number, and sensor id."
        << std::endl;
    return 1;
  }
  ...
  // create an http server running on a port
  auto&& exposer = Exposer{port_number};

  // send the target (url:port_number) to Prometheus server
